<template>
  ...
  <h3 v-if="isShow">{{ courseGoal }}</h3>
  <!-- Task 2: Toggle (show/ hide) the goal with help of the button  -->
  <button @click="showCourse">Toggle Goal</button>
...
export default {
  setup() {
    const courseGoal = ref('Apply composition API with Vue3 and liferay');
    ...
    return { courseGoal, isShow,  showCourse };
  },
};
</script>
